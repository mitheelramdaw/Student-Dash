...
def dashboard_page():
    ...
    # Dropdown to select modules for filtering (Multiple selections allowed)
    modules = st.multiselect("Select Modules", df.columns[2:], default=df.columns[2:])

    # Slider for minimum score
    min_score = st.slider("Minimum Marks", min_value=0, max_value=100, value=50)

    # Filter students based on the selected modules and minimum marks
    filtered_df = df[df[modules].ge(min_score).any(axis=1)]

    # Displaying filtered student data
    st.dataframe(filtered_df.style.applymap(lambda x: 'background-color: yellow' if x >= min_score else '', subset=modules))

    # Plotting the best scores
    plot_best_scores(filtered_df, program, modules)
...
def plot_best_scores(df, program, modules):
    ...
    for idx, student in df.iterrows():
        ...
        # Create the hover text for each module, including student name and marks
        hover_text = [f"{module}: {student[module]} marks" for module in modules]

        # Adding the hover text and additional information
        fig.add_trace(go.Bar(
            x=modules,
            y=student[modules],
            name=student_name,
            marker=dict(color=color),
            hovertext=[f"{student_name}<br>{', '.join(hover_text)}"],  # Show student name + marks + module
